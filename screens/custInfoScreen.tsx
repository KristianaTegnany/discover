...
export const custInfoScreen = ({ route, navigation }: Props) => {
    ...
    if (date.isSame(moment.tz("America/Martinique"), "day")) {
      if ([TAKEAWAY, DELIVERY].includes(bookingType)) {
        if (stopYesterday) {
          ...
          isValid =
            moment.tz(date, "America/Martinique").diff(dateNightstart) < 0 &&
      ...
      if (menu.attributes.provisionStockBase.length > 0) {
        let provision = menu.attributes.provisionStockBase.filter((x: any) =>
          moment
            ...
            .isSame(moment.tz(day, "America/Martinique"), "day")
        );
        if (provision[0]) {
  ...
  async function goPay() {
    ...
    if (!email || EmailValidator.validate(email) == false) {
    ...
    else if (bookingType == 'Delivery' && !(line1 && line1.length > 5 && zip && /^[0-9]{5}$/.test(zip))) {
      if (!line1 || line1.length < 6)
        Alert.alert(
          "Veuillez saisir une adresse valide"
        );
      else if (!zip || !/^[0-9]{5}$/.test(zip))
        Alert.alert(
      ...
      if (testOC && testOD && testDelayCren && testNoonNight && testQty) {
        ...
        resaRaw.set("delifare", bookingType == 'Delivery' ? Number(delifare) : 0);
        ...
        <View>
          ...
          {intcust && intcust.paymentChoice == "stripeOptin" && (
            <View style={styles.appButtonTextContainer}>
              <Text style={styles.appButtonText}>Avec{' '}</Text>
              <Image
                source={require("../assets/images/pplogo2.png")}
                fadeDuration={0}
