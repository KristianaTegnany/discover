    ...
    add: (state, action) => {
      ...
      if (stateNew.length == 0) {
        ...
        stateNew.forEach((productExistant) => {
          ...
          if (
            productExistant &&
            productExistant.persoData &&
            isEqual(action.payload.persoData, productExistant.persoData)
          ) {
          ...
          if (
            productExistant &&
            productExistant.formulaChoiced &&
            isEqual(
              action.payload.formulaChoiced,
              productExistant.formulaChoiced
            )
          ) {
          ...
          let IndexRaw = stateNew.findIndex(
            (productEx) =>
              productEx.id == action.payload.id &&
              isEqual(
                action.payload.formulaChoiced,
                productEx.formulaChoiced
              ) &&
              isEqual(action.payload.persoData, productEx.persoData)
          );
    ...
    remove: (state, action) => {
      ...
      stateNew.map((productExistant) => {
        ...
        if (
          productExistant &&
          productExistant.formulaChoiced &&
          isEqual(action.payload.formulaChoiced, productExistant.formulaChoiced)
        ) {
          checkIfformulaChoicedMatch = true;
        }

        if (
          productExistant.id == action.payload.id &&
          checkIfformulaChoicedMatch == true &&
          checkIfPersoDataMatch == true &&
          ...
          stateNew = stateNew.filter(
            (prodEx) =>
              prodEx.id !== action.payload.id &&
              isEqual(action.payload.formulaChoiced, prodEx.formulaChoiced) &&
